...
var games map[string]*onirim.Game
...
func apiNewGame(w http.ResponseWriter, r *http.Request) {
	...
	if games == nil {
		games = make(map[string]*onirim.Game)
	}
	games[g.ID] = g
...
type apiGetHandler func(*onirim.Game, http.ResponseWriter, *http.Request) ([]byte, error)
...
func apiGetStatus(game *onirim.Game, w http.ResponseWriter, r *http.Request) ([]byte, error) {
	s := <-game.NextStatus
	resp := struct {
		End    bool
...
func apiGetBoard(game *onirim.Game, w http.ResponseWriter, r *http.Request) ([]byte, error) {
	<-game.Ready
	return json.Marshal(game.GetBoard())
}

func apiGetPrompt(game *onirim.Game, w http.ResponseWriter, r *http.Request) ([]byte, error) {
	p := <-game.NextPrompt
	resp := struct {
		End    bool
